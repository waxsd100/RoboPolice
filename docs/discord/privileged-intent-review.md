...
内容はデータベースへの書き込み前に AES-256 で暗号化され、30日で自動的に削除されます。単一メッセージの
削除がログに出力された場合は、30日を待たずにその時点で該当行を削除します。また管理者は
...
### 8.3 削除後の挙動（運用上のトレードオフ）

30日を過ぎた行が消えると、そのメッセージに対する操作は**ログに一切出なくなります**。
「内容不明」といった代替の埋め込みすら出ず、完全に無言です。

| 操作 | 30日以内 | 30日経過後 |
|---|---|---|
| メッセージ削除 | 本文付きでログ出力 | **何も出力されない**（`messageDelete.js:19` で `return`） |
| メッセージ編集 | Before/After をログ出力 | **何も出力されない**（`messageUpdate.js:20` で `return`） |
| 一括削除 | paste へ書き出してリンク | **何も出力されない**（`getMessagesByIds` が `null` を返し `paste()` が即 return） |

**モデレーション上の意味**: 31日以上前の問題のある発言は、削除しても痕跡が残りません。
保持期間を延ばせばこの穴は狭まりますが、31日以上にするとフォームの「30日以内か」に Yes と
答えられなくなります。ここは**プライバシーとモデレーション能力の直接のトレードオフ**です。

> BOT側の告知はこの挙動に合わせてあります。`/help` は「N日より古いメッセージは削除・編集しても
> ログが出ない」と表示し、`/clearmydata` は「N日で自動削除される」と表示します。
> `MESSAGE_HISTORY_DAYS` の値を変えると表示も自動で追従します。

*改善案（未実装）*: 行が見つからない場合に「メッセージが削除されました（保持期間を過ぎているため
内容は保存されていません）」という埋め込みを出すようにすれば、少なくとも**削除された事実**は
記録できます。3ファイルの `return` を分岐に変える程度の変更です。必要なら実装します。
