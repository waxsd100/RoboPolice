...
招待の作成と削除、ボイスチャンネルの入室・退室・移動、メンバーのカスタムステータスの変更。
...
```
本アプリはメンバーが「カスタムステータス（Custom Status）」を変更した際に、その変更履歴をログに
出力する機能を持っています。これは、メンバーが自身のカスタムステータスに詐欺サイト・フィッシングのリンクや、
禁止ワード・不適切な発言を設定することを監視するために使用されます。

この機能は、設定されたカスタムステータス（type: 4 の Activity）のみを監視の対象としています。
単なるオンラインやオフライン等の状態遷移や、プレイ中のゲームに関する情報など、他の Presence 情報は
ログに出力せず、データベースにも保存していません。
カスタムステータスの変更があった場合のみ、変更前後のステータステキストをモデレーター専用のログチャンネルへ
送信します。
```
