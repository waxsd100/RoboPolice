...
サーバーに追加することはできません。設置されているのは当方が運営するサーバーのみで、実際の
モデレーション業務を行っているコミュニティサーバー1つと、変更を本番へ反映する前に動作確認する
非公開のテストサーバー数個です。前者の運営スタッフが自サーバーのモデレーションのために運用して
います。1万ユーザーの基準に達したのは、そのコミュニティサーバーのメンバーが1万人を超えている
ためであり、多数のサーバーに配布されているためではありません。
...
最小化しています。保存するのはメッセージID、投稿者ID、タイムスタンプ、および AES-256 で暗号化した
本文と添付URLのみで、サーバーIDとチャンネルIDは保存していません。また単一メッセージの削除が
...
| 対象 | 期限 | 実装 |
|---|---|---|
| PostgreSQL `messages` の行 | `MESSAGE_HISTORY_DAYS` 日（既定30） | `src/miscellaneous/prune.js` |
