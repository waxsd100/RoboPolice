		...
		if (listIgnoredChannelsOption) {
					...
					embeds: [
						{
							...
							description:
								global.bot.guildSettingsCache[interaction.guildID].ignoredChannels
									.map((id) => {
										return `<#${id}> (${id})`;
									})
			...
			interaction.createMessage({
				embeds: [
					{
						title: 'Success',
						description: `I am ${isDisabled ? 'ignoring' : 'logging'} events in <#${
							channelOption?.value || interaction.channel.id
						}>`,
						thumbnail: {
							url: interaction.member.user.dynamicAvatarURL(null, 64),
						},
