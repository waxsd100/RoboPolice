...
module.exports = {
	...
	func: async (interaction) => {
		...
		if (shouldStopLoggingEverything) {
			...
			const eventsLoggingHere = global.bot.guildSettingsCache[interaction.guildID].eventLogByNames(
				channelToStopLogging || interaction.channel.id,
			);
				...
				interaction
					.createMessage({
						embeds: [
							{
								title: 'Warning',
								description: `No events were logging to <#${
									channelToStopLogging || interaction.channel.id
								}>. Use \`/setup list\` to review the current logging setup.`,
								thumbnail: {
									url: interaction.member.user.dynamicAvatarURL(null, 64),
								},
			...
			interaction
				.createMessage({
					embeds: [
						{
							description: `All events logging to <#${channelToStopLogging || interaction.channel.id}> (${
								channelToStopLogging || interaction.channel.id
							}) have been unset.`,
							thumbnail: {
								url: interaction.member.user.dynamicAvatarURL(null, 64),
