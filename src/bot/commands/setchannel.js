...
module.exports = {
	func: async (message, suffix) => {
		...
		if (!botPerms.manageWebhooks || !botPerms.viewAuditLogs) {
			message.channel
				.createMessage(
					'I need manage webhooks and view audit logs permissions to run setchannel! This is necessary for me to send messages to your configured logging channel.',
				)
				.catch((_) => {});
		...
		if (events.length === 0 && suffix) {
			message.channel.createMessage(
				`<@${message.author.id}>, none of the provided events are valid. Look at ${process.env.GLOBAL_BOT_PREFIX}help to see what is valid.`,
			);
			...
			message.channel.createMessage(
				`<@${message.author.id}>, I set all events to log here! ${
					!botPerms.manageChannels || !botPerms.manageGuild
						? "Invite tracking will not work until I'm granted manage channels & manage server (I cannot get invite information without both!)"
						: ''
				}`,
			);
		} else {
			...
			await guildWebhookCacher(message.channel.guild.id, message.channel.id);
			message.channel.createMessage(
				`<@${message.author.id}>, it has been done. ${
					events.includes('guildMemberAdd') && (!botPerms.manageChannels || !botPerms.manageGuild)
						? "Invite tracking will not work until I'm granted manage channels & manage server (I cannot get invite information without both!)"
						: ''
				}`,
			);
	...
	name: 'setchannel',
	quickHelp:
		'The [dashboard](https://logger.bot) is the easiest way to setup! Setchannel configures bot logging behavior.',
  ...
  \`${
		process.env.GLOBAL_BOT_PREFIX
	}setchannel guildMemberAdd, guildMemberRemove, guildMemberKick\` <- joins, leaves, kicks logging **(YOU MUST ALLOW LOGGER __MANAGE CHANNELS AND MANAGE SERVER__ FOR INVITE TRACKING TO WORK! Why? Discord does not send invite info without it!)**
  \`${
		process.env.GLOBAL_BOT_PREFIX
