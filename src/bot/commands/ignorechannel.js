...
module.exports = {
	func: async (message, suffix) => {
		...
		if (suffix && !isNaN(parseInt(suffix))) {
			...
			if (!channelToIgnore || !(channelToIgnore.type === 2 || channelToIgnore.type === 0))
				return message.channel.createMessage(
					`Usage: ${process.env.GLOBAL_BOT_PREFIX}ignorechannel OR ${process.env.GLOBAL_BOT_PREFIX}ignorechannel channelID`,
				);
			toIgnore = suffix;
		}
		...
		const respStr = `Toggled logging events targeting <#${toIgnore}> (${
			message.channel.guild.channels.get(toIgnore).name
		}). I am now ${disabled ? 'ignoring' : 'logging'} events from this channel`;
