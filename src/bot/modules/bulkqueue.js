...
module.exports = async (senderPkg, guildSettings) => {
	if (webhookIDToQueue.has(senderPkg.webhookID)) {
		...
		if (updatedQueue.length < 10 && webhookIDToCharLimit.get(senderPkg.webhookID) + charsToPush <= 5000) {
			...
			webhookIDToTimeout.set(
				senderPkg.webhookID,
				setTimeout(() => {
					sendBulkLog(
						senderPkg,
						webhookIDToQueue
							.get(senderPkg.webhookID)
							.map((p) => p.embeds)
							.flat(1),
						guildSettings,
					);
		...
		} else if (updatedQueue.length === 10 || webhookIDToCharLimit.get(senderPkg.webhookID) + charsToPush >= 5000) {
			// the limit is 6000 but let's be safe
...
function sendBulkLog(senderPkg, embeds, guildSettings) {
	global.bot
		.executeWebhook(senderPkg.webhookID, senderPkg.webhookToken, {
		...
		.catch(async (e) => {
			...
			if (e && e.code && !(e.code == '50035' || e.code == '10015' || e.code == '500')) {
				global.logger.warn(
					`Got ${e.code} while sending webhook to ${senderPkg.guildID} (${
						global.bot.guilds.get(senderPkg.guildID)
							? global.bot.guilds.get(senderPkg.guildID).name
