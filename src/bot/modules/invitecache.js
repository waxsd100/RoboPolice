...
module.exports = {
	...
	cacheInvitesWhole: async (guildID, invitesArray) => {
		...
		await global.redis.set(
			`linvites-${guildID}`,
			JSON.stringify(invitesArray.map((i) => module.exports.formatInvite(i, true))),
			'EX',
			10800000,
		);
	},
	insertInvite: async (guildID, invite) => {
		...
		if (!invites) {
			await global.redis.set(
				`linvites-${guildID}`,
				JSON.stringify([module.exports.formatInvite(invite, true)]),
				'EX',
				10800000,
			);
		} else {
			...
			await global.redis.set(
				`linvites-${guildID}`,
				JSON.stringify(invites.map((i) => module.exports.formatInvite(i, true))),
				'EX',
				10800000,
			);
		}
	...
	formatInvite: (invite, doEncrypt) => {
		// strip useless info and only keep what I want
