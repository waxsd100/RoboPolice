...
function processCommand(message, commandName, suffix) {
	...
	if ((command.noDM || command.perm || command.type === 'admin') && !message.channel.guild) {
		...
		command.noThread &&
		(message.channel.type === 10 || message.channel.type === 11 || message.channel.type === 12)
	) {
		message.channel.createMessage('You cannot use this command in a thread!');
		return;
	} else if (message.author.id === process.env.CREATOR_IDS) {
		global.logger.info(
			`Developer override by ${message.author.username}#${message.author.discriminator} at ${new Date().toUTCString()}`,
		);
		...
		command.type === 'admin' &&
		!message.member.permissions.has('administrator' || message.author.id === message.channel.guild.ownerID)
	) {
		message.channel.createMessage("That's an admin only command. You need the administrator permission to use it.");
		return;
	} else if (
		command.perm &&
		!(message.member.permissions.has(command.perm) || message.author.id === message.channel.guild.ownerID)
	) {
		message.channel.createMessage(
			`This command requires you to be the owner of the server, or have the ${command.perm} permission.`,
		);
		return;
	} else if (command.perms && command.perms.find((p) => !message.member.permissions.has(p))) {
		message.channel.createMessage(
			`This command requires you to be the owner of the server, or have the following permissions: ${command.perms.join(
				', ',
			)}`,
		);
		return;
	...
	global.logger.info(
		`${message.author.username}#${message.author.discriminator} (${message.author.id}) in ${message.channel.id} sent ${commandName} with the args "${suffix}". The guild is called "${message.channel.guild.name}", owned by ${message.channel.guild.ownerID} and has ${message.channel.guild.memberCount} members.`,
	);
