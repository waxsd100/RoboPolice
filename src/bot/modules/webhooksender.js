...
const doNotAggregate = ['disconnect', 'error', 'interactionCreate', 'inviteCreate', 'inviteDelete', 'warn'];
...
module.exports = async (pkg) => {
	...
	if (
		!global.bot.getChannel(guildSettings.getEventByName(pkg.eventName))?.permissionsOf(global.bot.user.id).json
			.manageWebhooks ||
		!global.bot.getChannel(guildSettings.getEventByName(pkg.eventName)).permissionsOf(global.bot.user.id).json
			.viewAuditLog
	)
		...
		if (guild.memberCount < 10000 && guild.voiceStates.size < 100) {
			global.bot
				.executeWebhook(webhookID, webhookToken, {
					...
					embeds: pkg.embeds,
					allowedMentions: {
						// even though this is an embed and cannot ping, why not
						everyone: false,
						roles: false,
						users: false,
					},
				...
				.catch(async (e) => {
					if (
						...
						(e.message.includes('Request timed out') ||
							e.message.includes('503 Service Temporarily') ||
					...
					if (
						...
						!(
							...
							e.code == '500' ||
							e.code == '503' ||
							(e && e.message && e.message.includes('Internal Server Error'))
						)
					) {
						global.logger.warn(
							`Got ${e.code} while sending webhook to ${pkg.guildID} (${
								global.bot.guilds.get(pkg.guildID) ? global.bot.guilds.get(pkg.guildID).name : 'Could not find guild!'
							})`,
						);
