...
global.bot.on('rawREST', (r) => {
	...
	if (r.url.includes('webhooks')) {
		...
		if (Date.now() - lastWebhookTs > 5000 && webhookHitCount > 1000) {
			// 1000 webhook sends in 5 seconds is a lot
			global.logger.warn('Webhook activity heuristic', r.url, r.body);
			if (webhookHitCount >= 2000) {
				// dump the flooding requests for some time
				webhookHitCount = 0;
			}
