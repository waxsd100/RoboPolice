...
module.exports = async (guildID, channelID) => {
  ...
  try {
    ...
    global.logger.warn(`Log channel ${channelID} in ${guildID} may not exist even though it is in cache, resetting log configuration to this channel`, e)
    await global.redis.del(`webhook-${channelID}`)
    global.bot.guildSettingsCache[guildID].clearEventByID(channelID)
    await clearEventByID(guildID, channelID)
    await cacheGuild(guildID)
  ...
  if (!logChannel) {
    ...
    const newHook = await global.bot.createChannelWebhook(logChannel.id, {
