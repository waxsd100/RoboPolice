...
module.exports = async (guildID, channelID) => {
	...
	for (let i = 0; i < webhooks.length; i++) {
		if (
			webhooks[i].token &&
			webhooks[i].channel_id === channelID &&
			webhooks[i].application_id === global.bot.user.id
		) {
			// check for token because channel subscriptions count as webhooks
	...
	if (!logChannel) {
		global.logger.warn(
			`Logchannel ${channelID} in ${global.bot.guilds.get(guildID).name} ${guildID} does not exist low`,
		);
		await global.redis.del(`webhook-${channelID}`);
		global.bot.guildSettingsCache[guildID].clearEventByID(channelID);
		await clearEventByID(guildID, channelID);
		...
		const newHook = await logChannel
			.createWebhook(
				{
