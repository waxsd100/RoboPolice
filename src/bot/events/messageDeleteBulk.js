...
async function paste(messages, guildID) {
	...
	const pasteString = messages
		...
		.map((m) => {
			...
			return `${globalUser.username}#${globalUser.discriminator} (${m.author_id}) | (${
				globalUser.avatarURL
			}) | ${new Date(m.ts).toUTCString()}: ${m.content}`;
		})
		.join('\r\n');
	if (pasteString) {
			...
			.end((err, res) => {
				if (!err && res.body && res.statusCode === 200 && res.body.key) {
					messageDeleteBulkEvent.embeds[0].fields.push({
						name: 'Link',
						value: `${process.env.PASTE_CREATE_ENDPOINT}/${res.body.key}.txt`,
					});
