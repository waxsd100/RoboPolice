...
module.exports = () => {
	...
	global.bot.on('global-ratelimit-hit', (timeLeft) => {
		...
		global.webhook.error(
			`${new Date().toISOString()} [${cluster.worker.rangeForShard}] global ratelimit hit, time remaining: ${timeLeft}`,
		);
		console.warn(
			`${new Date().toISOString()} [${cluster.worker.rangeForShard}] global ratelimit hit, time remaining: ${timeLeft}`,
		);
	...
	global.bot.on('ratelimit-hit', (info) => {
		...
		console.warn(
			`${new Date().toISOString()} [${cluster.worker.rangeForShard}] ratelimit hit, is ${!info.global && 'not '}global`,
			info.info,
		);
	});
		...
		if (webhookErrorCount % 10 === 0) {
			console.warn(
				`${new Date().toISOString()} [${
					cluster.worker.rangeForShard
				}] webhook ratelimit error mod 10 hit rt after | error count | webhook id`,
				lastRetryAfter,
				webhookErrorCount,
				d.webhookID,
			);
