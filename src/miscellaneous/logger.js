...
module.exports = {
	...
	info: (...info) =>
		console.log(chalk`${new Date().toUTCString()} {bold.blue INFO}: `, ...info.map((m) => chalk`{white ${m}}`)),
	debug: (...info) =>
		console.log(chalk`${new Date().toUTCString()} {bgGreen DEBUG}: `, ...info.map((m) => chalk`{black.bgWhite ${m}}`)),
};
